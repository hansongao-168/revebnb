...
## 12. 二期（已实现 2026-05-18）

- `listing_calendar_feeds.merge_into_site_availability`（默认 `false`，per-feed 开关）
- `BookingAvailabilityService::externalCalendarNightSet()` 合并进 `unavailableNightSetForSiteCalendar` 与 `assertBookingAllowed`
- Filament feed 表单/表格展示「合并进前台可订」

**房东**：`/landlord-portal` 房源编辑页可进入只读「日历对比」（不可配置 ICS）

**租户**：`/tenant-admin` 房源编辑页可进入只读「日历对比」（不可配置 ICS）
